  ...
  <div class="add-form">
    <div class="form-item" v-for="(attr, idx) in aForm" :key="idx">
      <template v-if="attr.isLabel">
        ...
        >
          <el-option
            v-for="item in kgStore.dictLabels"
            :key="item.value"
            :label="item.label"
            :value="item.value"
        /></el-select>
      ...
      <template v-else>
        ...
        >
          <el-option
            v-for="item in kgStore.dictAttrs"
            :key="item.value"
            :label="item.label"
            :value="item.value"
        /></el-select>
...
import { checkExistSysLabel, checkExistSysProp } from "@/utils/kg";
...
/* 属性列表 */
const aForm = ref([
  { label: "标签", value: [], immutableAttr: true, isLabel: true },
]);
/* 新增属性 */
function addProp(idx) {
  aForm.value.splice(idx + 1, 0, {
    label: "",
    value: "",
    immutableAttr: false,
  });
...
async function submit() {
  ...
  const isAdded = await kgStore.addNode(aForm.value);
  if (isAdded) {
    ElMessage("节点创建成功");

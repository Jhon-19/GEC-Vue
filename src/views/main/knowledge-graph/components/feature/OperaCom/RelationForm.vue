  ...
  <div class="relation-form">
    <el-form>
      ...
      <el-form-item label="关系名称">
        ...
        >
          <el-option
            v-for="item in kgStore.dictRelations"
            :key="item.value"
            :label="item.label"
            :value="item.value"
        /></el-select>
...
import { transNodes } from "@/utils/kg";
import {
...
const queryNode = async (queryString, cb) => {
  let results = [];
  if (queryString) {
    const records = await kgStore.queryNodeByIdName(queryString);
    results = transNodes(records);
  }
  cb(results);
};
...
async function submit() {
  ...
  if (
    !targetNode.value ||
    targetNode.value[SYSTEM_NODE_NAME] !== rForm.target
  ) {
  ...
  const isCreated = await kgStore.createRelation(info);
  if (isCreated) {
    ElMessage.success("关系创建成功");
    kgStore.setNeedReDraw(true);
    // 清空表单
    for (let k in rForm) {
      if (k !== "from") {
        rForm[k] = null;

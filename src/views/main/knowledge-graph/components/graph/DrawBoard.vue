    ...
    <el-popover trigger="click" :width="270" v-model:visible="showPopoer">
      ...
      <div class="relation-detail">
        ...
        <div class="relation-opera" style="text-align: right">
          <el-button v-if="curRelation.isEdit" @click="cancelEdit"
            >取消</el-button
          >
          <el-button
            type="primary"
            v-if="curRelation.isEdit"
            @click="submitRelation"
            >保存</el-button
          >
          <el-button type="primary" v-else @click="editRelation"
            >编辑</el-button
          >
...
<script setup>
import { ref, reactive, onMounted, onUnmounted, watch, toRef } from "vue";
import {
  ElMessageBox,
  ElMessage,
  ElInput,
...
function delNode() {
  ElMessageBox.confirm("确认删除节点?", "提示", {
    ...
    .then(async () => {
      const isDeleted = await kgStore.deleteNode(selectedId.value);
      if (isDeleted) {
...
function showEdgeetail() {
  ...
  tmpStoreRelation[SYSTEM_RELATION_COMMENT] =
    curRelation.value.props[SYSTEM_RELATION_COMMENT];
...
async function submitRelation() {
  ...
  if (needExcute) {
    const isEdited = await kgStore.editRelation(params);
    if (isEdited) {
      ElMessage.success("关系编辑成功");
      emits("edit-edge");
    } else {
      ElMessage.error("关系编辑失败");
    }
  }
  curRelation.value.isEdit = false;
}
...
function delRelation() {
  ElMessageBox.confirm("确认删除关系?", "提示", {
    ...
    .then(async () => {
      const isDeleted = await kgStore.deleteEdge(selectedEdge.value);
      if (isDeleted) {
        ElMessage("关系删除成功");
        emits("delete-edge");
      } else {
...
.float-info {
  ...
  &.fixed-pos {
    ...
    transition: top var(--starry-sky-transition-time),
      left var(--starry-sky-transition-time);
  }
  .node-info {
